    ...
    def generate(
        ...
        **kwargs,
        ...
        for _ in range(n_window_steps):
            ...
            return output_coarse[:, input_coarse_len:]
...
class BarkModel(BarkPreTrainedModel):
    ...
    def generate(
        ...
        **kwargs,
        ...
        # TODO: no fine speaker embedding yet

        # reshape (seq_len, n_coarse_codebooks)
        coarse_output = coarse_output.view(coarse_output.shape[0], -1, coarse_generation_config.n_coarse_codebooks)

        # brings ids into the range [0, codebook_size -1]
        value = torch.remainder(coarse_output - semantic_generation_config.semantic_vocab_size, self.generation_config.codebook_size)
        
        # transpose: (B, L, K) -> (K, B, L)
        value = value.transpose(0,2).transpose(1,2)
        
        
        value = self.vocos.codes_to_features(value)

        value = self.vocos.decode(value, bandwidth_id=torch.tensor([2]).to(self.device))
